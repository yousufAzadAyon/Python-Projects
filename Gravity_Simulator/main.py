...
def update_height():
    ...
    for i in range(1, 5):
        heights["height_%d" % i].config(
            text="Height: "
            + str(round(415 - main_canvas.coords(balls["ball_%d" % i])[3], 2))
        )
...
def step(t):
    ...
    for i in range(1, 5):
        ...
        data["data_%d" % i].append(
            (time, 415 - main_canvas.coords(balls["ball_%d" % i])[3])
        )
...
def run():
    ...
    while (
        15 < main_canvas.coords(balls["ball_1"])[3] < 415
        or 15 < main_canvas.coords(balls["ball_2"])[3] < 415
        or 15 < main_canvas.coords(balls["ball_3"])[3] < 415
        or 15 < main_canvas.coords(balls["ball_4"])[3] < 415
    ):
        step(t_slider.get())
...
def reset():
    ...
    for i in range(1, 5):
        ...
        main_canvas.coords(
            balls["ball_%d" % i], 45 + (i - 1) * 100, 405, 55 + (i - 1) * 100, 415
        )
...
heights = {}
for i in range(1, 5):
    heights["height_%d" % i] = tkinter.Label(
        input_frame,
...
# Time slider
t_slider = tkinter.Scale(
    input_frame, from_=0, to=1, tickinterval=0.1, resolution=0.01, orient=HORIZONTAL
)
...
step_button = tkinter.Button(
    input_frame, text="Step", command=lambda: step(t_slider.get())
)
